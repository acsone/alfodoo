...
class CmisProxy(http.Controller):
    ...
    def _get_requests_timeout(self):
        ...
        """
        try:
            timeout = float(self.env['ir.config_parameter'].get_param(
    ...
    def _apply_permissions_mapping(
        ...
        if not model_inst:
            return
        all_allowable_actions = [
            aa for aa in gen_dict_extract("allowableActions", value)
        ]
    ...
    def _prepare_json_response(
        ...
        self._clean_url_in_dict(
            value,
            urllib.parse.urlparse(cmis_location).geturl(),
        ...
        response = werkzeug.Response(
            json.dumps(value), mimetype="application/json", headers=headers
        )
        return response
    ...
    def _decode_token(self, cmis_path, proxy_info, params, token):
        ...
        if not model_inst.exists():
            _logger.info(
                "The referenced model doesn't exist or the user has "
                "no read access (%s, %s)",
                model,
                res_id,
            )
    ...
    def _check_cmis_content_access(
