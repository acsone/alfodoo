...
(function () {
    ...
    function estimateTypeByHeaderContentType(documentUrl, httpHeaders, cb) {
    ...
    window.onload = function () {
        var viewer;
        var urlParts = document.location.hash.substring(1).split('||');
        var documentUrl = urlParts[1];
        var httpHeaders = JSON.parse(decodeURIComponent(urlParts[0]));
        var location = document.createElement('a');
        location.href = documentUrl;
        var parameters = parseSearchParameters(location);
        parameters.httpHeaders = httpHeaders;
        ...
        if (documentUrl) {
            ...
            estimateTypeByHeaderContentType(documentUrl, httpHeaders, function(pluginData) {
                if (!pluginData) {
                    if (parameters.type) {
                ...
                if (pluginData) {
                    ...
                    if (plugin.constructor.name === "PDFViewerPlugin"){
                        var old_initialize = plugin.initialize; 
                        plugin.initialize = function(viewContainer, location) {
                            ...
                            old_initialize.call(plugin, viewContainer, dataLocation);
                        }
                    }
                    viewer = new Viewer(plugin, parameters);
